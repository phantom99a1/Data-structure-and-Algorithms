namespace Longest_Unequal_Adjacent_Groups_Subsequence_I
{
    /*You are given a string array words and a binary array groups both of length n, where words[i] is associated with groups[i].
    Your task is to select the longest alternating subsequence from words. A subsequence of words is alternating if for any 
    two consecutive strings in the sequence, their corresponding elements in the binary array groups differ. 
    ...
    class Program
    {
        static void Main()
        {
            ...
            int[] groups = { 1, 0, 1, 1 };
            Console.WriteLine(string.Join(", ", sol.GetLongestSubsequence(words, groups))); // Output: "a, b, c"
        }
    }
}
